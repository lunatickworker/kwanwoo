...
import { Wallet, Plus, X, RefreshCw, TrendingUp, TrendingDown, Copy, Check, Trash2, ArrowUpRight, Send, AlertCircle, Loader2 } from 'lucide-react';
...
export function MasterProfileCard({ onClose }: MasterProfileCardProps) {
  const { user } = useAuth();
    ...
    >
      ...
      {!showWithdrawForm && (
        ...
        <div className="flex items-center justify-between p-6 border-b border-cyan-500/20">
          <div>
            <h3 className="text-xl text-cyan-400 font-semibold">{user?.username || 'Master'}</h3>
            <p className="text-sm text-slate-400 mt-1">마스터 관리자</p>
          </div>
          <button
            onClick={onClose}
