...
export function SettlementManagement() {
  ...
  const fetchSettlementData = async () => {
    try {
      ...
      const settlementData: CenterSettlement[] = centers?.map(center => {
        ...
        const feeRate = (center.fee_rate || 3.0) / 100; // 센터 기본 수수료 3%
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-sm text-slate-500 text-center p-4 bg-slate-800/30 rounded-lg">
        <Eye className="w-4 h-4 inline-block mr-2" />
        읽기 전용 모드 - 정산 데이터는 마스터가 관리합니다
      </div>
    </div>
  );
}
